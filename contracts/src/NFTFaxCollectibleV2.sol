...
/// @title NFTFaxCollectibleV2
/// @notice V2 of the Base mint for the NFTfax chain-letter game — adds
...
///     someone else.
/// Mitigations kept on-chain: per-(community, sourceTokenId) single claim,
/// an optional global per-address mint cap, and Pausable so the owner can
/// keep the contract paused outside active game windows.
///
...
contract NFTFaxCollectibleV2 is ERC721, ERC2981, Ownable, Pausable, ReentrancyGuard {
    ...
    /// communities. 0 = uncapped. A lightweight anti-sybil backstop on top
    /// of the per-(community, sourceTokenId) single-claim rule.
    ...
    /// Prevents the same (community, sourceTokenId) from minting twice,
    /// so a single Chonk/Normie/etc. can only claim one collectible.
    mapping(Community => mapping(uint256 => bool)) public claimed;
    ...
    function _mintChonk(
        ...
        if (claimed[Community.CHONK][sourceTokenId]) revert AlreadyClaimed();

        address to = _resolveChonkRecipient(sourceTokenId);
        _checkMintCap(to);

        claimed[Community.CHONK][sourceTokenId] = true;
        mintsReceived[to] += 1;
    ...
    function _mintDirect(
        ...
        if (msg.value < mintPrice) revert InsufficientPayment();
        if (totalMinted >= MAX_SUPPLY) revert MaxSupplyReached();
        if (claimed[community][sourceTokenId]) revert AlreadyClaimed();
        _checkMintCap(to);

        claimed[community][sourceTokenId] = true;
        mintsReceived[to] += 1;
        mintedTokenId = ++totalMinted;
        _safeMint(to, mintedTokenId);
        if (bytes(tokenURI_).length > 0) {
